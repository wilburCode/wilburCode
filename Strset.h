...
public:
   Strset(void); //for copies
   Strset(const char *nm);
   Strset(const char *nm,const char *pnam); //pnam is suffix of path file
   Strset(int n,const char *nm); //n gets appended to type if >-1
   ~Strset();
   void SetMem(Strset &Stst); //Sets data & memory to Stst data & memory
   ...
   bool own_strset_mem; //Does the instance own the memory maps, false if from SetMem
   int num_file; //To remember number used to open file or set to -1
};

class Lexos : public Strset {
public:
   Lexos(void); //for copies
   Lexos(const char *nm);
   Lexos(const char *nm,const char *pnam); //pnam is suffix of path file
   ...
   void create_Lexos(strMap &Mp,long m);
...
public:
   LexCnt(const char *);
   LexCnt(const char *,const char *pnam); //pnam is suffix of path name file.
   ~LexCnt(void);
...
public:
   ...
   LexPrb(int mn,const char *,const char *pnam); //pnam is suffix of path name file.
   ~LexPrb(void);
...
public:
   LexDCnt(const char *);
   LexDCnt(const char *,const char *pnam); //pnam is suffix of path name file.
   ~LexDCnt(void);
...
public:
   ...
   LexPrb2(int mn,const char *,const char *pnam); //pnam is suffix of path name file.
   ~LexPrb2(void);
   void set_max(int mx); //Sets max.
