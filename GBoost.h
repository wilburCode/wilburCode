...
class GBoost3 {
public:
  GBoost3(long nd,long t,long *tn); //nd whole space
  ...
  void update_store(double *sco, const char *nam); //sco is pointer at
     //score array. nam is name under which to store
     //the Isgrid results.
  double Del_entropy(long n,float *sco); //If n is not in training set
  ...
  double Del_error(long n,float *sco); //If n is not in training set
     //Computes the expected change in error rate on unseen docs
     //if it is labelled and added to the training set.
     //sco is the score produced by Vnak scoring of doc n
  double Gain_Ratio(long n,float *sco); //If n is not in training set
  ...
  long *trn; //Stores the training set +1, -1, 0

  long grn; //Granularity, default 10,000.
  long t; //Number of the iteration.
  double lam; //Factor to scale down old scores
  double zprod; //Product of Zs
  double epsilon; //Limit on p.
  Isgrid *pIsg; //Pointer at Isgrid object last used in update.
};
