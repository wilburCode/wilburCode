...
class FBase {
   public:
      ...
      void mak_Msync(const char *a,char *ptr);
         //Syncronizes the file and the map for ptr based on path and name composition.
      void mak_Msync(long n,const char *a,char *ptr);
      ...
      void rm_File(const char *a); //remove the file with *a extension
      ...
      //Writes local time stamp in file with "time" extension
