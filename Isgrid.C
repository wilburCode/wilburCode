...
double Isgrid::ave_prec(void){
   ...
   for(i=0;i<n;i++) pr[i]=rev[i]/tot[i];
   pr[n]=pr[n-1];
   double xx=0;
   for(i=0;i<n;i++) {
      xx+=(pr[i]+pr[i+1])*0.5*a[i];
   }
   double av=xx/(sum_r);
   delete [] rev;
   delete [] tot;
...
double Isgrid::prcrec_BreakEven(void){
   long i,n=0,j;
   for(i=0;i<num;i++){
      if(*(xd+i)>0){
         *(sr+n)=(double)i;
   ...
   double h=TotP-sum_t+s[R];
   double rem=h*((double)a[R]/(double) s[R]);
   double xx=(sum_r + rem);
   return (xx/TotP);
...
void Isgrid::split(long npp,long *idx){
   ...
   *(b+i-1)=kp=i-1;
   ...
   if(pflag)cout << nm2 << " " << nm1 << " " << npp << endl;
...
void Isgrid::write_3df(void){
   ...
   *pfout << zb << " " << ze << " " << zdel << endl;
   ...
   dst_Ostr(pfout);
   if(pflag)cout << xsize << " by " << ysize << " by " << zsize << " grid of points" << endl;
}
...
void Isgrid::write_2df(void){
   ...
   *pfout << yb << " " << ye << " " << ydel << endl;
   ...
   dst_Ostr(pfout);
   if(pflag)cout << xsize << " by " << ysize << " grid of points" << endl;
}
...
void Isgrid::SetMem2(Isgrid &CIg){
   num=  CIg.num;
   xsize=CIg.xsize;
   xb=   CIg.xb;
   xe=   CIg.xe;
...
void Isgrid::SetMem1(Isgrid &CIg){
   num=  CIg.num;
   xsize=CIg.xsize;
   xb=   CIg.xb;
   xe=   CIg.xe;
...
double Isgrid::inv_1df(double x){
   long i,j,k;

   if(x<=p[0])return(xb);
   if(p[xsize-1]<x)return(xe);
   i=0;
   j=xsize-1;
   while(j-i>1){
      k=(i+j)/2;
...
double Isgrid::inu_1df(double x){
   long i,j,k;

   if(x<=p[0])return(xb);
   if(p[xsize-1]<x)return(xb+(xsize-1)*xdel);
   i=0;
   j=xsize-1;
   while(j-i>1){
      k=(i+j)/2;
...
long Isgrid::index_1df(double x){
   long i,j,k;
...
void Isgrid::extend_2df(void){
   ...
   for(j=xsize-2;j>=0;j--){
      ...
      if(*(xd+id)==0)*(p+id)=*(p+id-1);
         ...
         if(pflag)cout << *(p+id) << " > " << *(p+id-1) << endl;
         exit(0);
      }
   }
   for(i=ysize-2;i>=0;i--){
      ...
      if(*(xd+id)==0)*(p+id)=*(p+id-xsize);
         ...
         if(pflag)cout << *(p+id) << " > " << *(p+id-xsize) << endl;
         exit(0);
      }
   }
   for(i=ysize-2;i>=0;i--){
   ...
   for(j=1;j<xsize;j++){
      ...
      if(*(xd+id)==0)*(q+id)=*(q+id+1);
         ...
         if(pflag)cout << *(q+id) << " < " << *(q+id+1) << endl;
         exit(0);
      }
   }
   for(i=1;i<ysize;i++){
      ...
      if(*(xd+id)==0)*(q+id)=*(q+id+xsize);
         ...
         if(pflag)cout << *(q+id) << " < " << *(q+id+xsize) << endl;
         exit(0);
      }
   }
   for(i=1;i<ysize;i++){
