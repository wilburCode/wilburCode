...
GBoost3::GBoost3(long nd,long t,long *tn){
   ndoc=nd;
   pdoc=NULL;
   ts_sxx=NULL;
   tdoc=t;
...
void GBoost3::init(double eps,long gr){
   long i,j;
   epsilon=eps;
   grn=gr;
...
void GBoost3::update(double *sco){
   int pflag=get_qflag();
   long i,j,k;
   double dmax,dmin,xx,yy,sum,deps;
...
void GBoost3::update(float *sco){
   ...
   pIsg->init1();
   for(i=0;i<ndoc;i++){
      if(trn[i]==1)pIsg->add_data((double)sco[i],pdoc[i],pdoc[i]);
      else if(trn[i]==-1)pIsg->add_data((double)sco[i],pdoc[i],0.0);
   }
   pIsg->dim1();
...
void GBoost3::update_store(double *sco,const char *nam){
   int pflag=get_qflag();
   long i,j,k;
   double dmax,dmin,xx,yy,sum,deps;
...
double GBoost3::Del_entropy(long n,float *sco){
   int pflag=get_qflag();
   long i,j,k;
   double Ppos,Pneg,del=0,ps;
   double dmax,dmin,xx,yy,deps,uu,pre,prf;
...
double GBoost3::Del_error(long n,float *sco){
   int pflag=get_qflag();
   long i,j,k;
   double Ppos,Pneg,del=0,ps;
   double dmax,dmin,xx,yy,deps,uu,pre,prf;
...
double GBoost3::Gain_Ratio(long n,float *sco){
   int pflag=get_qflag();
   long i,j,k;
   double Ppos,Pneg,del=0,ps;
   double dmax,dmin,xx,yy,deps,uu,vv,prf;
   double sum,ddx=0.01;
