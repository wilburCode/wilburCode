...
void PostBrill_FN::gopen_PostBrill_FN(void){
  ...
  tagstr[0]='\0';
  no_tagstr[0]='\0';
  get_pathw(cnam,"postbrillset","gene","multi");
  ...
  fin.close();
  fin.open("postbrillset_gene.mg",ios::in);
  while(fin.getline(s, XSTR, '\n')) {
    Ten_PM.add_key_count(s);
  }
  fin.close(); 
  ...
  fin.open(cnam, ios::in);
  if(!fin.is_open()){cout << cnam << " failed to open" << endl;exit(0);}
  while(fin.getline(cnam, XSTR, '\n')) { 
    ppre    = (pcre*) pcre_malloc (XSTR);
    ppre    = pcre_compile(cnam, 0, 
