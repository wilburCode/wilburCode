...
void FBase::rm_File(const char *a){
   char cnam[max_str];
   get_pathx(cnam,a);
   string sx="rm ";
   sx+=cnam;
...
char *FBase::get_Mmap(const char *a){
   ...
   ::close(fld);
//cout << "Mnmap " << cnam << " " << hex << (long)ptr << dec << endl;
   return(ptr);
}
...
void FBase::mak_Msync(long n,const char *a,char *ptr){
   struct stat datf;
   char cnam[max_str];
   if(ptr==NULL){cout << "NULL pointer" << endl;return;}
   get_pathx(cnam,n,a);
...
void FBase::time_Stamp(void){
   ofstream *pfout=get_Ostr("time",ios::out);
   time_t timv=time(0);
   struct tm *local_tm=localtime(&timv);
   *pfout << "local time stamp: " << asctime(local_tm);
   dst_Ostr(pfout);
}
