...
Strset::Strset(void) : FBase("strset","null"){
   own_strset_mem = false;
}
...
Strset::Strset(const char *nam,const char *pnam) : FBase("strset",nam,pnam){
   own_strset_mem = false;
}
...
Strset::~Strset(void){
   if(own_strset_mem){
      if(num_file>-1)gclose_map(num_file);
      else gclose_map();
   }
}

void Strset::SetMem(Strset &Stst){
   num= Stst.num;
   addr=Stst.addr;
   str =Stst.str;
}
...
Lexos::Lexos(const char *nam,const char *pnam) : Strset(nam,pnam){
   this->change_type("lexos");
   tx=new char[max_str];
   ht=new long[max_str];
   sn=new long[max_str];
...
void Lexos::create_Lexos(strMap &Mp,long m){
   long i=0;
   this->gopen_write(m);
   typename strMap::iterator p=Mp.begin();
   typename strMap::iterator q=Mp.end();
...
LexCnt::LexCnt(const char *nam,const char *pnam) : Lexos(nam,pnam){
   this->change_type("lexcnt");
}
...
LexPrb::LexPrb(int mn,const char *nam,const char *pnam) : LexCnt(nam,pnam){
   this->change_type("lexprb");
   min=mn;
}
...
LexDCnt::LexDCnt(const char *nam,const char *pnam) : Lexos(nam,pnam){
   this->change_type("lexcnt");
}
...
LexPrb2::LexPrb2(int mn,const char *nam,const char *pnam) : LexDCnt(nam,pnam){
   this->change_type("lexprb");
   min=mn;
}
