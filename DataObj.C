...
void Index::Save(const char *nam,const char *pnam){
   FBase Fb("index",nam,pnam);
   ofstream *pfout=Fb.get_Ostr("x",ios::out);
   write(*pfout);
   Fb.dst_Ostr(pfout);
}

void Index::Load(const char *nam,const char *pnam){
   FBase Fb("index",nam,pnam);
   ifstream *pfin=Fb.get_Istr("x",ios::in);
   read(*pfin);
   Fb.dst_Istr(pfin);
...
WOrder::WOrder(Order *pOrd,float *weg){
   ...
   long i,n=pInd->ix;
   weight=new float[n];
   for(i=0;i<n;i++)weight[i]=weg[pInd->idx[order[i]]];
   delete pOrd;
}
...
WOrder::WOrder(Order *pOrd,double *weg){
   ...
   long i,n=pInd->ix;
   weight=new float[n];
   for(i=0;i<n;i++)weight[i]=(float)weg[pInd->idx[order[i]]];
   delete pOrd;
}
...
float WOrder::dice_sco(WOrder *wpord){
   ...
   long i,j,k;
   float s1=0,s2=0,sc1=0,sc2=0;
   float xi,yj;
   long m=pInd->ix;
   long n=wpord->pInd->ix;

   Index *pJnd=pInd->cbool_And(wpord->pInd);
   if(!pJnd)return(0);
   for(i=0;i<m;i++){
      s1+=score[i];
      k=ind(i,xi);
...
int WOrder::subset_equal(int n, WOrder *wpord){

   long i,j,k,v;
   int flag;
   float xi,yj;
   long m=pInd->ix;
   Index *pJnd=wpord->pInd;
   long r=pJnd->ix;
   if((m<n)||(r<n))return(0);
