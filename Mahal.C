...
Mahal::~Mahal(){
  if(cov_g!=NULL){
    long r;
    for(r=0;r<dim;r++) delete cov_g[r];
    delete [] cov_g;
  }
  if(cov_b!=NULL){
    long r;
    for(r=0;r<dim;r++) delete cov_b[r];
    delete [] cov_b;
  }
  if(mean_g!=NULL) delete [] mean_g;
  if(mean_b!=NULL) delete [] mean_b;
...
void Mahal::Learn_Mahal(void){
  ...
  det_b=pLud_b->det();
  coef=log(pri_g/pri_b)-0.5*log(det_g/det_b); 

  delete tInd, bInd;
  delete pLud_g, pLud_b;
  delete [] sum_g;
  delete [] sum_b;
}

void Mahal::Save_Mahal(){
  long r, c;
  ...
  *pfout<<"\t"<<coef<<endl; 
  ptfout->write((char*) &coef, sizeof(double));
  dst_Ostr(pfout);
  dst_Ostr(ptfout);
}
...
void Mahal::Load_Mahal(){
  ...
  if(dim==dmn){
    ...
    cov_g=new double*[dim];
    for(r=0;r<dim;r++) cov_g[r]=new double[dim];
    cov_b=new double*[dim];
    for(r=0;r<dim;r++) cov_b[r]=new double[dim]; 
    mean_g=new double[dim];
    mean_b=new double[dim];
