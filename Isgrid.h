...
public:
  ...
  ~Isgrid();
  void SetMem1(Isgrid &CIg); //for 1D case set all data & pointers to CIg data & pointers
  void SetMem2(Isgrid &CIg); //for 2D case set all data & pointers to CIg data & pointers
  ...
  long index_1df(double x); //Returns the index of the p value.
  void extend_1df(void); //Extends the function to all values by averaging
  double inv_1df(double x); //returns smallest y>=xb for which val_1df(y)>=x
     //if val_1df(xe)>=x, else returns xe.
  double inu_1df(double x); //returns largest y=xb+m*xdel<xe for which val_1df(y)<x
     //if val_1df(xb)<x, else returns xb.
  //These two functions are only valid to use for 1 dimensional data and should be called before dim1()
  double ave_prec(void);
  double prcrec_BreakEven(void);
...
//private:
  ...
  long pflag; //Print flag, 1 by default.
